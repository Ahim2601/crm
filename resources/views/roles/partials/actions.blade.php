@can('role.show')
<a href="{{ route('role.show', $id) }}" class="btn btn-sm btn-icon btn-text-info
    rounded-pill"
    data-bs-toggle="tooltip" title="Ver rol">
    <i class="ri-eye-line ri-20px"></i>
</a>
@endcan
@can('role.edit')
<a href="{{ route('role.edit', $id) }}" class="btn btn-sm btn-icon btn-text-secondary
    rounded-pill"
    data-bs-toggle="tooltip" title="Editar rol">
    <i class="ri-edit-2-line ri-20px"></i>
</a>
@endcan
@can('role.destroy')
<a href="javascript:;" class="btn btn-sm btn-icon btn-text-secondary
    rounded-pill text-danger"
    data-bs-toggle="tooltip" title="Eliminar rol"
    onclick="deleteRecord({{ $id }})">
    <i class="ri-delete-bin-7-line ri-20px"></i>
</a>
@endcan